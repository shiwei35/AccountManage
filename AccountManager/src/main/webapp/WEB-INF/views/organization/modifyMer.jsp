...
<script type="text/javascript">
	...
	function onSelect(date) { 
		$('#dMerexpdate').val(date.toLocaleDateString());  
	}
	function saveObj() { 
			...
			<table id="update" class="modifytable" width="100%">
				...
				<tr>
					...
					<td><input class="easyui-validatebox" name="sMername"
						...
						data-options="required:true" value="${merObj.sMername}" /></td>
				</tr>
				...
				<tr>
					...
					<td><input class="easyui-validatebox" name="sMerstatus"
						style="width: 400px" validType="length[1,50]"
						data-options="required:true" value="${merObj.sMerstatus}" /></td>
				</tr>
				</tr>
				<tr>
					<th align="center" width="35%">商户到期时间</th>
					<td><input name="dMerexpdate" id="dMerexpdate" 
						style="width: 400px" dateFormat="date" class="easyui-datebox" 
						value="${merObj.dMerexpdate}" required="required" editable="false"
						data-options="onSelect:onSelect" /> </td>
				</tr>
				<tr>
					<!-- 城市代码未知，后续加入字典表中 -->
				...
				<tr>
					<td colspan="2" align="center"><input type="button" value="保存"
						...
						type="hidden" name="iMercode" value="${merObj.iMercode}" /> <input
						type="hidden" name="flag" value="${flag}"> 
						<input type="hidden" name="sIseffect" value="${merObj.sIseffect}"></td>
				</tr>
			</table>
		</form>
	</div>
